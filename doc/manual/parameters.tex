...
`adiabatic boundary': An initial temperature condition that allows for discretizing the model domain into two layers separated by a user-defined isotherm using a table look-up approach. The user includes an input ascii data file that is formatted as 3 columns 'latitude', 'longitude', and 'depth', where 'depth' is in kilometers and represents that depth of an initial temperature of 1673.15 K. See Initial conditions/Adiabatic boundary for more details about the file formatting. The temperature is defined
from the surface (273.15 K) to the isotherm as a linear gradient. Below the isotherm the temperature increases adiabatically (0.0005 K per meter). This initial temperature condition is designed specifically for the ellipsoidal chunk geometry model.
