...
`adiabatic boundary': An initial temperature in which the base of the lithosphere is an isotherm boundary between conductive lithosphere and adiabatic geothermal gradient in the manthle. The temperature is a linear interpolation from the surface to an isotherm depth 1673.15 K at the base of the lithosphere, from which the temperature increases adiabatically (0.0005 K per meter). The lithosphere thickness model (for example from seismic tomography data) is read from a data file (text format) consist 3 columns 'latitude', 'longitude' and  'depth', where 'depth' [km] is the depth of base of the lithosphere for a given surface coordinates (latitude and longitude in degrees  in WGS84 reference frame).  This initial temperature condition is specifically for the ellipsoidal chunk geometry model. 
...
\label{parameters:Initial_21conditions/adiabatic_21boundary}

\begin{itemize}
\item {\it Parameter name:} {\tt Lithosphere Isotherm Filename}
\phantomsection\label{parameters:Initial conditions/adiabatic boundary/Lithosphere Isotherm Filename}


\index[prmindex]{Lithosphere Isotherm Filename}
\index[prmindexfull]{Initial conditions!adiabatic boundary!Lithosphere Isotherm Filename}
{\it Value:} \$ASPECT\_SOURCE\_DIR/data/initial-conditions/adiabatic\_boundary/litho1.0\_model.txt



{\it Default:} \$ASPECT\_SOURCE\_DIR/data/initial-conditions/adiabatic\_boundary/litho1.0\_model.txt


{\it Description:} File from which the base of the lithosphere depth data is read. Note that full path (\$DIRECTORY\_PATH/filename) of the location of the file must be provided. The data format consist of 3 colums: latitude[degree] longitude[degree] depth[km]. The data must be arranged such that the latitude is kept constant while running over all incremented longitudes, then the latitude is incremented or decremented on step further.
