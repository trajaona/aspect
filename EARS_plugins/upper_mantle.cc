      ...
      for (unsigned int i=0; i < in.position.size(); ++i)
        {
		  ...
		  AssertThrow(this->introspection().compositional_name_exists("middle_crust"),
				      ExcMessage("Material model Upper mantle works only works if there is a "
				                  "compositional field called crust."));
		  ...
		  const unsigned int upper_crust_idx = this->introspection().compositional_index_for_name("upper_crust")+1;
		  const unsigned int middle_crust_idx = this->introspection().compositional_index_for_name("middle_crust")+1;
		  const unsigned int lower_crust_idx = this->introspection().compositional_index_for_name("lower_crust")+1;
		  const unsigned int mantle_lithosphere_idx = this->introspection().compositional_index_for_name("mantle_lithosphere")+1;
		 // const unsigned int plate_boundaries_idx = this->introspection().compositional_index_for_name("plate_boundaries")+1;

          const std::vector<double> volume_fractions = compute_volume_fractions(composition);
          std::vector<double> densities(volume_fractions.size());
          double density = 0.0;
          const double temperature_factor = (1 - thermal_alpha * (in.temperature[i] - reference_T));

          densities[0] = reference_rho * temperature_factor;
          densities[upper_crust_idx]  =  ascii_data.get_data_component(surface_boundary_id, in.position[i], 4);
          densities[middle_crust_idx] =  ascii_data.get_data_component(surface_boundary_id, in.position[i], 5);
          densities[lower_crust_idx]  =  ascii_data.get_data_component(surface_boundary_id, in.position[i], 6);
          densities[mantle_lithosphere_idx] = reference_rho;
          for (unsigned int j=0; j < volume_fractions.size(); ++j)
               density += volume_fractions[j] * densities[j];
          out.densities[i] = density;

          composition_viscosities[upper_crust_idx]           =  crust_out.viscosities[i];
          composition_viscosities[middle_crust_idx]          =  crust_out.viscosities[i];
          composition_viscosities[lower_crust_idx]           =  crust_out.viscosities[i];
          composition_viscosities[mantle_lithosphere_idx]    =  std::min(std::max(dislocation_creep (pressure, temperature, in.strain_rate[i]), min_visc), max_visc);
          ...
          // Viscosity of the sublithospheric mantle.
          composition_viscosities[0] = (disl * diff) / (disl + diff);
