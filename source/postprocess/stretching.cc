...
#include <aspect/postprocess/stretching.h>
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    Stretching<dim>::execute (TableHandler &)
    {

      // evaluate of the solution gradients at all of our evaluation points
      std::vector<Tensor<1, dim>>
      current_point_gradient (this->introspection().n_components, Tensor<1,dim>());
      std::vector<Tensor<2,dim> >
      velocity_gradients (evaluation_points.size(), Tensor<2,dim>());
      std::vector<SymmetricTensor<2,dim>>
	  strain (evaluation_points.size(), SymmetricTensor<2,dim>());
      std::vector<Tensor<1,dim>>
	  principal_stretch (evaluation_points.size(), Tensor<1,dim>());
      std::vector<std::pair<double, double>>
      finite_strain(evaluation_points.size());
      Eigen::MatrixXf stretching_tensor (dim, dim);
      Eigen::MatrixXf eigen_values (dim, dim);
      Eigen::MatrixXf eigen_vectors (dim, dim);
      boost::numeric::ublas::matrix<double> F(dim,dim);
      ...
      for (unsigned int p=0; p<evaluation_points.size(); ++p)
        {
          ...
          // Reduce all collected values into local array
          for (unsigned int d=0; d< dim; ++d)
            velocity_gradients[p][d] = Utilities::MPI::sum
                                       ((n_procs >1)
                                        ?
                                        current_point_gradient[d] / n_procs
                                        :
                                        current_point_gradient[d],
      ...
      for (unsigned int p=0; p<evaluation_points.size(); ++p)
        {
          ...
          strain[p] = symmetrize(velocity_gradients[p]);
          ...
          for (unsigned int d = 0; d < dim; d++)
            principal_stretch[p][d] = eigen_vectors(d, idx.front());

          finite_strain[p] = std::make_pair(Utilities::compute_vector_azimuth_wrt_north<dim>
                                            (evaluation_points[p], principal_stretch[p]),
								            std::log(lambda[idx.front()] / lambda[idx.back()]));
        }

      //finally push these point values all onto the list we keep
      point_values.push_back (std::make_pair (this->get_time(), finite_strain));
      ...
      for (std::vector<std::pair<double, std::vector<std::pair<double, double>>  > >::iterator
          ...
          for (unsigned int i=0; i<evaluation_points.size(); ++i)
            {
              ...
              for (unsigned int d =0; d < dim; ++d)
                {
                  f << ellipsoidal_evaluation_points[i][d];
                  f << "  ";
                }
              f << /* Stretching Azimuth */ time_point->second[i].first<<"  ";
              f << /* Natural strain */ time_point->second[i].second;
              f << '\n';
            }
        }
    ...
    template <int dim>
    void
    Stretching<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Stretching");
        {
    ...
    template <int dim>
    void
    Stretching<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Stretching");
    ...
    void Stretching<dim>::serialize (Archive &ar, const unsigned int)
    {
    ...
    template <int dim>
    void
    Stretching<dim>::save (std::map<std::string, std::string> &status_strings) const
    {
      std::ostringstream os;
      aspect::oarchive oa (os);
      oa << (*this);

      status_strings["Stretching"] = os.str();
    }


    template <int dim>
    void
    Stretching<dim>::load (const std::map<std::string, std::string> &status_strings)
    {
      // see if something was saved
      if (status_strings.find("Stretching") != status_strings.end())
        {
          std::istringstream is (status_strings.find("Stretching")->second);
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(Stretching,
                                  "stretching",
                                  "A postprocessor that azimuth of principal strain of the deformation tensor")
  }
}
