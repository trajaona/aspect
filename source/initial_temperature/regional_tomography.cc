...
namespace aspect
{
  namespace InitialTemperature
  {
    ...
    RegionalTomography<dim>::initial_temperature (const Point<dim> &position) const
    {
      ...
      wcoord[0] = depth;
      double temp = vs_to_temperature(pressure, Vs_look_up->get_shear_wave_velocity(wcoord), depth)+273;
      const double depth_linear = 75000;
      if (depth < depth_linear)
        {
          wcoord[0] = depth_linear;
          const double a = (vs_to_temperature(pressure, Vs_look_up->get_shear_wave_velocity(wcoord), depth_linear) - 273.15) / depth_linear;
          const double b = 350.15;
          //std::cout<<depth<<"   "<<temp<<std::endl;
          return temp = a * depth + b;
        }
      else
        //std::cout<<depth<<"   "<<temp<<std::endl;
        return temp;
