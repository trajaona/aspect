  ...
  namespace InitialTemperature
  {
   ...
   ascii_grid_vs (const Point<dim> &position) const
   {
    if (use_reference_profile)
       ...
       return  Utilities::AsciiDataInitial<dim>::get_data_component(position,1);
   }
    ...
    RegionalTomography<dim>::initial_temperature (const Point<dim> &position) const
    {
     ...
     double vs_perturbation = 0.0;
      const double topo = ascii_data_topo.get_data_component(surface_boundary_id, position, 0) * topo_fac;    
        ...
        double temp_factor = (oceanic_density == 3000.0)? 0.5 : 1;
        double temperature_perturbation = 0.0;
        double lab_reference_Vs;
        ...
        s.set_points(spline_depths,reference_Vs);
         ...
         if (use_reference_profile)
    	  ...
    	  // Set a maximum perturbation to 500 K and minimum -500 K. DO TO: Find a more resonnable perturbation value. 
           temperature_perturbation = (depth < 400000.0)? std::min(std::max((-1./thermal_alpha) * vs_to_density * vs_perturbation,-500.0), 500.0) : 0.0;
       ...
       lab_reference_Vs = s(isotherm_depth);
       ...
       double temp;
       if (depth <= 30000.0 - topo)
            // Temperature in the sticky air
            temp = 298.0;
       else  if (depth > ( 30000.0 - topo) && depth <= isotherm_depth + 30000.0)
            // Sometimes the temperature at the surface become negative so set it to 298 K. 
            temp = std::max(temp_factor * lithosphere_geotherm (depth- (30000.0 - topo), isotherm_depth, upper_crust_bottom, middle_crust_bottom, lower_crust_bottom, lab_temperature), 298.0);
       else 
            // Sublithospheric mantle temperature. Adiabtic gradient is 0.0003K/m. 
            temp =  lab_isotherm_temperature + (depth - 30000.0  - isotherm_depth) * 0.0003 + temperature_perturbation;
       return temp;
    }
