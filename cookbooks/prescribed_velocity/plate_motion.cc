...
namespace aspect
{
  ...
  {
    prm.declare_entry ("Prescribe plate motion", "false",
                       Patterns::Bool (),
    ...
    prm.enter_subsection ("Plate velocities");
    {
  ...
  void parse_parameters(const Parameters<dim> &,
                        ParameterHandler &prm)
  {
    prescribe_internal_velocities = prm.get_bool ("Prescribe plate motion");
    prm.enter_subsection ("Plate velocities");
    {
      prm.enter_subsection("Velocity function");
      {
        ...
        for (cell = simulator_access.get_dof_handler().begin_active();
          ...
          if (! cell->is_artificial())
                    ...
                    !current_constraints.is_constrained(local_dof_indices[q]))
                  {
                    ...
                    if ((c_idx >=
                         ...
                         simulator_access.introspection().component_indices.velocities[dim-1]))
                      {
                        ...
                        const double temperature = simulator_access.get_initial_temperature_manager().initial_temperature(p);
			const double depth                       = simulator_access.get_geometry_model().depth(p);
                        ...
                        if (temperature < 1680 && temperature > 1665)
//                        if (depth <= 125000)
                          {
